            ...
            <div class="bg-gradient-to-r from-slate-100 to-gray-100 rounded-xl shadow-lg p-6 mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <a href="{{ route('yer-sotuvlar.list', $periodInfo['period'] !== 'all' ? ['period' => $periodInfo['period'], 'year' => $periodInfo['year'], 'quarter' => $periodInfo['quarter'] ?? null, 'month' => $periodInfo['month'] ?? null] : []) }}"
                        ...
                        <p class="text-3xl font-bold text-purple-700 mb-1">{{ number_format($summaryTotal['total_lots']) }} та</p>
                    ...
                    <a href="{{ route('yer-sotuvlar.list', $periodInfo['period'] !== 'all' ? ['period' => $periodInfo['period'], 'year' => $periodInfo['year'], 'quarter' => $periodInfo['quarter'] ?? null, 'month' => $periodInfo['month'] ?? null] : []) }}"
                        ...
                        <p class="text-3xl font-bold text-indigo-700 mb-1">{{ number_format($summaryTotal['expected_amount'] / 1000000000, 2) }} млрд</p>
                    ...
                    <a href="{{ route('yer-sotuvlar.list', $periodInfo['period'] !== 'all' ? ['period' => $periodInfo['period'], 'year' => $periodInfo['year'], 'quarter' => $periodInfo['quarter'] ?? null, 'month' => $periodInfo['month'] ?? null] : []) }}"
                        ...
                        <p class="text-3xl font-bold text-teal-700 mb-1">{{ number_format($summaryTotal['received_amount'] / 1000000000, 2) }} млрд</p>
                    ...
                    <a href="{{ route('yer-sotuvlar.list', $periodInfo['period'] !== 'all' ? ['period' => $periodInfo['period'], 'year' => $periodInfo['year'], 'quarter' => $periodInfo['quarter'] ?? null, 'month' => $periodInfo['month'] ?? null] : []) }}"
                        ...
                        <p class="text-3xl font-bold text-amber-700 mb-2">{{ number_format($totalQoldiq / 1000000000, 2) }} млрд</p>
                        <div class="flex items-center mb-3">
                            <div class="flex-1 bg-gray-200 rounded-full h-2.5 mr-3">
                                <div class="bg-amber-600 h-2.5 rounded-full transition-all duration-500" style="width: {{ min(100, $totalQoldiqFoizi) }}%"></div>
                            </div>
                            <span class="text-sm font-bold text-amber-600">{{ number_format($totalQoldiqFoizi, 1) }}%</span>
                        </div>
                        @if ($periodInfo['period'] !== 'all')
                            <div class="mt-auto pt-3 border-t border-slate-200">
                                <p class="text-xs text-blue-600 font-medium flex items-center">
                        ...
                        <table class="w-full">
                            ...
                            <tfoot
                                ...
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-right text-sm text-green-700">
                                        {{ number_format(collect($tumanStatsMuddatliEmas)->sum('received') / 1000000000, 2) }}
                                    </td>
                                    <td
                                        ...
                                        @php
                                            ...
                                            $percentageFinal =
                                                $totalExpectedFinal > 0
                                                    ...
                                                    : 0;
                                        @endphp
                                        {{ number_format($percentageFinal, 1) }}%
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
