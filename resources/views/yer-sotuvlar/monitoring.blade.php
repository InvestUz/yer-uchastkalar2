            ...
            <div class="bg-gradient-to-r from-slate-100 to-gray-100 rounded-xl shadow-lg p-6 mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <a href="{{ route('yer-sotuvlar.list', $periodInfo['period'] !== 'all' ? ['period' => $periodInfo['period'], 'year' => $periodInfo['year'], 'quarter' => $periodInfo['quarter'] ?? null, 'month' => $periodInfo['month'] ?? null] : []) }}"
                        ...
                        <p class="text-3xl font-bold mb-1" style="color: rgb(29 78 216);">{{ number_format($summaryTotal['total_lots']) }} та</p>
                    ...
                    <a href="{{ route('yer-sotuvlar.list', $periodInfo['period'] !== 'all' ? ['period' => $periodInfo['period'], 'year' => $periodInfo['year'], 'quarter' => $periodInfo['quarter'] ?? null, 'month' => $periodInfo['month'] ?? null] : []) }}"
                        ...
                        <p class="text-3xl font-bold mb-1" style="color: rgb(29 78 216);">{{ number_format($summaryTotal['expected_amount'] / 1000000000, 2) }} млрд</p>
                    ...
                    <a href="{{ route('yer-sotuvlar.list', $periodInfo['period'] !== 'all' ? ['period' => $periodInfo['period'], 'year' => $periodInfo['year'], 'quarter' => $periodInfo['quarter'] ?? null, 'month' => $periodInfo['month'] ?? null] : []) }}"
                        ...
                        <p class="text-3xl font-bold mb-1" style="color: rgb(29 78 216);">{{ number_format($summaryTotal['received_amount'] / 1000000000, 2) }} млрд</p>
                    ...
                    <a href="{{ route('yer-sotuvlar.list', $periodInfo['period'] !== 'all' ? ['period' => $periodInfo['period'], 'year' => $periodInfo['year'], 'quarter' => $periodInfo['quarter'] ?? null, 'month' => $periodInfo['month'] ?? null] : []) }}"
                        ...
                        <p class="text-3xl font-bold mb-2" style="color: rgb(185 28 28);">{{ number_format($totalQoldiq / 1000000000, 2) }} млрд</p>
                        <div class="flex items-center mb-3">
                            <div class="flex-1 bg-gray-200 rounded-full h-2.5 mr-3">
                                <div class="h-2.5 rounded-full transition-all duration-500" style="width: {{ min(100, $totalQoldiqFoizi) }}%; background-color: rgb(185 28 28);"></div>
                            </div>
                            <span class="text-sm font-bold" style="color: rgb(185 28 28);">{{ number_format($totalQoldiqFoizi, 1) }}%</span>
                        </div>
                        @if ($periodInfo['period'] !== 'all')
                            <div class="mt-auto pt-3 border-t border-slate-200">
                                <p class="text-xs text-blue-600 font-medium flex items-center">
            ...
            <div id="content-muddatli" class="tab-content">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    ...
                    <a href="{{ route('yer-sotuvlar.list', array_merge(['tolov_turi' => 'муддатли'], $periodInfo['period'] !== 'all' ? ['period' => $periodInfo['period'], 'year' => $periodInfo['year'], 'quarter' => $periodInfo['quarter'] ?? null, 'month' => $periodInfo['month'] ?? null] : [])) }}"
                        ...
                        <p class="text-3xl font-bold mb-1" style="color: rgb(185 28 28);">{{ number_format($summaryMuddatli['total_lots']) }}
                            та</p>
                        ...
                        <div class="flex items-center mb-3">
                            <div class="flex-1 bg-gray-200 rounded-full h-2.5 mr-3">
                                <div class="h-2.5 rounded-full transition-all duration-500"
                                    style="width: {{ 100 - min(100, $qoldiqFoizi) }}%; background-color: rgb(185 28 28);"></div>
                            </div>
                            <span
                                class="text-sm font-bold" style="color: rgb(185 28 28);">{{ number_format(100 - $qoldiqFoizi, 1) }}%</span>
                        </div>
                                        ...
                                        @if($periodInfo['period'] === 'quarter')
                                            ...
                                            @forelse($selectedYearQuarters as $quarter)
                                                <option value="{{ $quarter['chorak_raqam'] }}"
                                                    ...
                                                    {{ $quarter['chorak_nomi'] }} ({{ number_format($quarter['summa'] / 1000000000, 2) }} млрд)
                                                </option>
                                            @empty
                                        ...
                                        @if($periodInfo['period'] === 'month')
                                            ...
                                            @forelse($selectedYearMonths as $month)
                                                <option value="{{ $month['oy'] }}"
                                                    ...
                                                    {{ $month['oy_nomi'] }} ({{ number_format($month['summa'] / 1000000000, 2) }} млрд)
                                                </option>
                                            @empty
                                                <option value="">Маълумот йўқ</option>
                                            @endforelse
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
                    ...
                    <div
                        ...
                        <div class="flex items-center">
                            <div class="flex-1 bg-gray-200 rounded-full h-2.5 mr-3">
                                <div class="h-2.5 rounded-full transition-all duration-500"
                                    style="width: {{ min(100, $summaryMuddatliEmas['payment_percentage']) }}%; background-color: rgb(29 78 216);"></div>
                            </div>
                            <span
                                class="text-sm font-bold" style="color: rgb(29 78 216);">{{ number_format($summaryMuddatliEmas['payment_percentage'], 1) }}%</span>
                        </div>
                    </div>
