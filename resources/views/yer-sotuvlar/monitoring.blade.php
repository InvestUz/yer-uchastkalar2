...
<div class="max-w-[98%] mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
        <form method="GET" action="{{ route('yer-sotuvlar.monitoring') }}" class="flex flex-wrap items-end gap-4">
            <div class="flex-1 min-w-[200px]">
                <label class="block text-sm font-semibold text-slate-700 mb-2">Аукцион санаси (дан)</label>
                <input type="date" name="auksion_sana_from" value="{{ $dateFilters['auksion_sana_from'] ?? '' }}"
            ...
            <div class="flex-1 min-w-[200px]">
                ...
                <input type="date" name="auksion_sana_to" value="{{ $dateFilters['auksion_sana_to'] ?? '' }}"
                       class="w-full px-4 py-2.5 border-2 border-slate-300 rounded-lg focus:ring-4 focus:ring-blue-200 focus:border-blue-500">
            </div>
            ...
            @if(!empty($dateFilters['auksion_sana_from']) || !empty($dateFilters['auksion_sana_to']))
                <a href="{{ route('yer-sotuvlar.monitoring') }}" class="px-6 py-2.5 bg-slate-200 hover:bg-slate-300 text-slate-800 rounded-lg font-semibold transition-all duration-300">
                    Тозалаш
                </a>
            @endif
        </form>
    ...
    <div class="mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-red-500">
                ...
                <p class="text-3xl font-bold" style="color: rgb(185, 28, 28);">{{ number_format($summaryMuddatli['total_lots']) }} та</p>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-blue-500">
                ...
                <p class="text-3xl font-bold" style="color: rgb(29, 78, 216);">{{ number_format($summaryMuddatli['expected_amount'] / 1000000000, 2) }} млрд</p>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-green-500">
                ...
                <p class="text-3xl font-bold" style="color: rgb(29, 78, 216);">{{ number_format($summaryMuddatli['received_amount'] / 1000000000, 2) }} млрд</p>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-purple-500">
                ...
                <p class="text-3xl font-bold" style="color: rgb(29, 78, 216);">{{ number_format($summaryMuddatli['payment_percentage'], 1) }}%</p>
            </div>
        </div>
                ...
                <table class="w-full">
                    ...
                    <tfoot class="bg-slate-100 font-bold">
                        <tr>
                            ...
                            <td class="px-4 py-3 text-center text-sm" style="color: rgb(185, 28, 28);">{{ number_format(collect($tumanStatsMuddatli)->sum('lots')) }}</td>
                            <td class="px-4 py-3 text-right text-sm" style="color: rgb(29, 78, 216);">{{ number_format(collect($tumanStatsMuddatli)->sum('grafik') / 1000000000, 2) }}</td>
                            <td class="px-4 py-3 text-right text-sm" style="color: rgb(29, 78, 216);">{{ number_format(collect($tumanStatsMuddatli)->sum('fakt') / 1000000000, 2) }}</td>
    ...
    <div class="mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-red-500">
                ...
                <p class="text-3xl font-bold" style="color: rgb(185, 28, 28);">{{ number_format($summaryMuddatliEmas['total_lots']) }} та</p>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-blue-500">
                ...
                <p class="text-3xl font-bold" style="color: rgb(29, 78, 216);">{{ number_format($summaryMuddatliEmas['expected_amount'] / 1000000000, 2) }} млрд</p>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-green-500">
                ...
                <p class="text-3xl font-bold" style="color: rgb(29, 78, 216);">{{ number_format($summaryMuddatliEmas['received_amount'] / 1000000000, 2) }} млрд</p>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-purple-500">
                ...
                <p class="text-3xl font-bold" style="color: rgb(29, 78, 216);">{{ number_format($summaryMuddatliEmas['payment_percentage'], 1) }}%</p>
            </div>
        </div>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-slate-200">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-4 py-3 text-sm font-semibold text-slate-800">{{ $stat['tuman'] }}</td>
                            <td class="px-4 py-3 text-center text-sm font-bold" style="color: rgb(185, 28, 28);">{{ number_format($stat['lots']) }}</td>
                            <td class="px-4 py-3 text-right text-sm font-semibold" style="color: rgb(29, 78, 216);">{{ number_format($stat['expected'] / 1000000000, 2) }}</td>
                            <td class="px-4 py-3 text-right text-sm font-semibold" style="color: rgb(29, 78, 216);">{{ number_format($stat['received'] / 1000000000, 2) }}</td>
                            <td class="px-4 py-3 text-right text-sm font-semibold {{ $stat['difference'] > 0 ? 'text-red-600' : 'text-green-600' }}">
                                {{ number_format($stat['difference'] / 1000000000, 2) }}
                            </td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <span class="text-sm font-bold" style="color: rgb(34, 197, 94);">{{ number_format($stat['percentage'], 1) }}%</span>
                                </div>
                            </td>
                        </tr>
                        @endforeach
                    ...
                    <tfoot class="bg-slate-100 font-bold">
                        <tr>
                            <td class="px-4 py-3 text-sm text-slate-800">ЖАМИ</td>
                            <td class="px-4 py-3 text-center text-sm" style="color: rgb(185, 28, 28);">{{ number_format(collect($tumanStatsMuddatliEmas)->sum('lots')) }}</td>
                            <td class="px-4 py-3 text-right text-sm" style="color: rgb(29, 78, 216);">{{ number_format(collect($tumanStatsMuddatliEmas)->sum('expected') / 1000000000, 2) }}</td>
                            <td class="px-4 py-3 text-right text-sm" style="color: rgb(29, 78, 216);">{{ number_format(collect($tumanStatsMuddatliEmas)->sum('received') / 1000000000, 2) }}</td>
