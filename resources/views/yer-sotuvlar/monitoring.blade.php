            ...
            <div class="space-y-2.5">
                ...
                <div class="flex items-center justify-between p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg hover:from-green-100 hover:to-emerald-100 transition-all duration-300 border border-green-200">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="font-semibold text-slate-800 text-sm">{{ $stat['tuman'] }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <div class="w-20 bg-green-200 rounded-full h-2">
                            <div class="bg-gradient-to-r from-green-500 to-green-600 h-2 rounded-full" style="width: {{ min($stat['percentage'], 100) }}%"></div>
                        </div>
                        <span class="text-base font-bold text-green-600 min-w-[60px] text-right">{{ number_format($stat['percentage'], 1) }}%</span>
                    </div>
                </div>
                @endforeach
            </div>
                ...
                <div class="flex items-center justify-between p-3 bg-gradient-to-r from-red-50 to-rose-50 rounded-lg hover:from-red-100 hover:to-rose-100 transition-all duration-300 border border-red-200">
                    ...
                    <div class="flex items-center gap-2">
                        <div class="w-20 bg-red-200 rounded-full h-2">
                            <div class="bg-gradient-to-r from-red-500 to-red-600 h-2 rounded-full" style="width: {{ min($stat['percentage'], 100) }}%"></div>
                        </div>
                        <span class="text-base font-bold text-red-600 min-w-[60px] text-right">{{ number_format($stat['percentage'], 1) }}%</span>
                    </div>
                </div>
                @endforeach
            </div>
            ...
            <table class="w-full">
                ...
                <tfoot class="bg-slate-100 font-bold">
                    <tr>
                        ...
                        <td class="px-4 py-3 text-center text-sm" style="color: rgb(185, 28, 28);">{{ number_format(collect($tumanStats)->sum('lots')) }}</td>
                        <td class="px-4 py-3 text-right text-sm" style="color: rgb(29, 78, 216);">{{ number_format(collect($tumanStats)->sum('grafik') / 1000000000, 2) }}</td>
                        <td class="px-4 py-3 text-right text-sm" style="color: rgb(29, 78, 216);">{{ number_format(collect($tumanStats)->sum('fakt') / 1000000000, 2) }}</td>
                        <td class="px-4 py-3 text-right text-sm {{ collect($tumanStats)->sum('difference') > 0 ? 'text-red-600' : 'text-green-600' }}">
                            {{ number_format(collect($tumanStats)->sum('difference') / 1000000000, 2) }}
                        </td>
...
new Chart(monthlyCtx, {
    ...
    data: {
        ...
        datasets: [{
            label: 'График',
            data: {!! json_encode($chartData['monthly']['grafik']) !!},
            ...
            label: 'Факт',
            data: {!! json_encode($chartData['monthly']['fakt']) !!},
...
new Chart(tumanCtx, {
    type: 'bar',
    data: {
        labels: {!! json_encode($chartData['tuman']['labels']) !!},
        datasets: [{
            label: 'График',
            data: {!! json_encode($chartData['tuman']['grafik']) !!},
            ...
            label: 'Факт',
            data: {!! json_encode($chartData['tuman']['fakt']) !!},
