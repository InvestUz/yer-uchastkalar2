    ...
    <div class="max-w-[98%] mx-auto">
        ...
        <div class="bg-white rounded-lg shadow mb-6 border-t-4 border-blue-600">
            ...
            <div class="p-5 bg-gray-50">
                <form method="GET" action="{{ route('yer-sotuvlar.monitoring_mirzayev') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                    ...
                    <div class="flex items-end">
                        <a href="{{ route('yer-sotuvlar.monitoring_mirzayev') }}" class="w-full bg-gray-500 hover:bg-gray-600 text-white font-semibold py-2 px-4 rounded text-sm transition-colors text-center">
                            Тозалаш
                        </a>
                    </div>
                </form>
                    ...
                    <tbody>
                        ...
                        <tr style="background-color: #FEF3C7;">
                            ...
                            <td class="border border-gray-400 px-2 py-2.5 text-right font-bold text-gray-900">
                                {{ number_format($comparativeData['jami']['selected_month']['plan'] / 1000000, 1) }}
                            </td>
                            <td class="border border-gray-400 px-2 py-2.5 text-right font-bold text-red-700">
                                {{ number_format($comparativeData['jami']['selected_month']['fakt'] / 1000000, 1) }}
                            </td>
                            <td class="border border-gray-400 px-2 py-2.5 text-center font-bold text-gray-900">
                                {{ $comparativeData['jami']['selected_month']['percentage'] }}%
                            </td>

                            <!-- Year to Date Total -->
                            <td class="border border-gray-400 px-2 py-2.5 text-right font-bold text-gray-900">
                                {{ number_format($comparativeData['jami']['year_to_date']['plan'] / 1000000, 1) }}
                            </td>
                            <td class="border border-gray-400 px-2 py-2.5 text-right font-bold text-red-700">
                                {{ number_format($comparativeData['jami']['year_to_date']['fakt'] / 1000000, 1) }}
                            </td>
                            <td class="border border-gray-400 px-2 py-2.5 text-center font-bold text-gray-900">
                                {{ $comparativeData['jami']['year_to_date']['percentage'] }}%
                            </td>

                            <!-- Full Year Total -->
                            <td class="border border-gray-400 px-2 py-2.5 text-right font-bold text-gray-900">
                                {{ number_format($comparativeData['jami']['full_year']['plan'] / 1000000, 1) }}
                            </td>
                            <td class="border border-gray-400 px-2 py-2.5 text-right font-bold text-red-700">
                                {{ number_format($comparativeData['jami']['full_year']['fakt'] / 1000000, 1) }}
                            </td>
                            <td class="border border-gray-400 px-2 py-2.5 text-center font-bold text-gray-900">
                                {{ $comparativeData['jami']['full_year']['percentage'] }}%
                            </td>
                        </tr>

                        <!-- Tuman Rows -->
                        @foreach($comparativeData['tumanlar'] as $index => $tumanData)
                            <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-gray-50' }} hover:bg-blue-50">
                                <!-- Row Number -->
                                <td class="border border-gray-400 px-2 py-2 text-center font-medium text-gray-700">{{ $index + 1 }}</td>
                                
                                <!-- Tuman Name -->
                                <td class="border border-gray-400 px-2 py-2 font-medium text-gray-800">{{ $tumanData['tuman'] }}</td>

                                <!-- Selected Month -->
                                <td class="border border-gray-400 px-2 py-2 text-right text-gray-700">
                                    {{ number_format($tumanData['selected_month']['plan'] / 1000000, 1) }}
                                </td>
                                <td class="border border-gray-400 px-2 py-2 text-right font-semibold {{ $tumanData['selected_month']['fakt'] > 0 ? 'text-blue-700' : 'text-gray-700' }}">
                                    {{ number_format($tumanData['selected_month']['fakt'] / 1000000, 1) }}
                                </td>
                                <td class="border border-gray-400 px-2 py-2 text-center font-semibold text-gray-800">
                                    {{ $tumanData['selected_month']['percentage'] }}%
                                </td>

                                <!-- Year to Date -->
                                <td class="border border-gray-400 px-2 py-2 text-right text-gray-700">
                                    {{ number_format($tumanData['year_to_date']['plan'] / 1000000, 1) }}
                                </td>
                                <td class="border border-gray-400 px-2 py-2 text-right font-semibold {{ $tumanData['year_to_date']['fakt'] > 0 ? 'text-blue-700' : 'text-gray-700' }}">
                                    {{ number_format($tumanData['year_to_date']['fakt'] / 1000000, 1) }}
                                </td>
                                <td class="border border-gray-400 px-2 py-2 text-center font-semibold text-gray-800">
                                    {{ $tumanData['year_to_date']['percentage'] }}%
                                </td>

                                <!-- Full Year -->
                                <td class="border border-gray-400 px-2 py-2 text-right text-gray-700">
                                    {{ number_format($tumanData['full_year']['plan'] / 1000000, 1) }}
                                </td>
                                <td class="border border-gray-400 px-2 py-2 text-right font-semibold {{ $tumanData['full_year']['fakt'] > 0 ? 'text-blue-700' : 'text-gray-700' }}">
                                    {{ number_format($tumanData['full_year']['fakt'] / 1000000, 1) }}
                                </td>
                                <td class="border border-gray-400 px-2 py-2 text-center font-semibold text-gray-800">
                                    {{ $tumanData['full_year']['percentage'] }}%
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            ...
            <div class="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
                ...
                <div class="space-y-1.5 text-xs">
                    <div class="flex justify-between">
                        ...
                        <span class="font-semibold text-gray-800">{{ number_format($comparativeData['jami']['selected_month']['plan'] / 1000000000, 2) }} млрд</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Факт:</span>
                        <span class="font-semibold text-blue-700">{{ number_format($comparativeData['jami']['selected_month']['fakt'] / 1000000000, 2) }} млрд</span>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
                <h3 class="text-sm font-bold text-gray-700 mb-3 pb-2 border-b border-gray-200">
                    {{ $comparativeData['meta']['selected_month'] }} ой якуни
                </h3>
                <div class="space-y-1.5 text-xs">
                    <div class="flex justify-between">
                        <span class="text-gray-600">План:</span>
                        <span class="font-semibold text-gray-800">{{ number_format($comparativeData['jami']['year_to_date']['plan'] / 1000000000, 2) }} млрд</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Факт:</span>
                        <span class="font-semibold text-blue-700">{{ number_format($comparativeData['jami']['year_to_date']['fakt'] / 1000000000, 2) }} млрд</span>
                    </div>
                    <div class="flex justify-between items-center pt-2 border-t border-gray-200">
                        <span class="text-gray-600">Тўланганлик:</span>
                        <span class="text-lg font-bold text-gray-800">
            ...
            <div class="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
                ...
                <div class="space-y-1.5 text-xs">
                    <div class="flex justify-between">
                        <span class="text-gray-600">План:</span>
                        <span class="font-semibold text-gray-800">{{ number_format($comparativeData['jami']['full_year']['plan'] / 1000000000, 2) }} млрд</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Факт:</span>
                        <span class="font-semibold text-blue-700">{{ number_format($comparativeData['jami']['full_year']['fakt'] / 1000000000, 2) }} млрд</span>
                    </div>
                    <div class="flex justify-between items-center pt-2 border-t border-gray-200">
                        <span class="text-gray-600">Тўланганлик:</span>
                        <span class="text-lg font-bold text-gray-800">
